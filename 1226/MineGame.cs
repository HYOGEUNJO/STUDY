...
namespace WhatIsFunction
{
...

    {
        ...
        // debug mode 에서 지뢰가 아닌 곳은 .(dot) 지뢰인 곳은 #(샵)으로 표현
        ...
        public static void Main()
        {
            ...
            while (isGameOver == false)
            {
                ...
                {
                    Console.WriteLine("[플레이어]  X 좌표 입력");
                    int.TryParse(Console.ReadLine(), out playerX);
                    Console.WriteLine("[플레이어]  Y 좌표 입력");
                    int.TryParse(Console.ReadLine(), out playerY);

                    // 플레이어가 입력한 좌표의 유효성을 검사한다.
                    isLocationValid = (0 <= playerX && playerX < BOARD_SIZE_X) && (0 <= playerY && playerY < BOARD_SIZE_Y);
                    if (isLocationValid == false)
                    {
                        Console.WriteLine("{0} {1}", "[System] 해당 좌표는 유효하지 않습니다.", "다른 좌표를 입력하세요 \n");
                        continue;
                    } // if : 좌표를 잘못 입력한 경우
                    // 좌표를 제대로 입력한 경우만 이 아래로 코드가 진행됨. 왜냐면 유효하지 않은 경우 위에서 continue 만나기 때문
                    ...
                    for (int y = 0; y < BOARD_SIZE_Y; y++)
                    {
                        for (int x = 0; x < BOARD_SIZE_X; x++)
                        {
                            ...
                            for (int searchY = y - 1; searchY <= y + 1; searchY++)
                            {
                                ...
                                {
                                    //유효하지 않은 좌표는 패스한다.
                                    isSearchTileValid = (0 <= searchX && searchX < BOARD_SIZE_X) && (0 <= searchY && searchY < BOARD_SIZE_Y);
                                    if (isSearchTileValid == false) { continue; }// 9타일 서치 중에 지뢰가 있다면 패스한다.
