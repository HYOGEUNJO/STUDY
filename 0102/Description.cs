...
namespace _0102
{
    ...
    {
        // 큰 규모의 프로그램일수록 뼈대를 구성하는 일이 중요하다. 인테페이스를 사용하면 전체 프로그램의 설계도에 대한 명세서를 작성할 수 있다.
        // 프로그램의 표준 규약을 정하고 따를 수 있도로 강제하는 인터페이스(Interface)를 학습한다. 인터페이스는 클래스에서 구현해야 하는 관련 기능의 정의가 포함된 개념이다.

        // <인터페이스>
        // 클래스 또는 구조체에 포홤될 수 있는 관련있는 메서드들을 묶어 관리한다. 인터페이스는 명세서(규약, 표준) 역할을 한다.
        // 인터페이스를 상속받아 그 내용을 구현하는 클래스는 인터페이스에 선언된 멤버(속성, 메서드 등)가 반드시 구현되어 있다는 보증을 한다.
    ...
    class Car : ICar
    {
        public virtual void Go()
        {
            ...
            Console.WriteLine("인터페이스를 사용항 떄는 정의된 모든 멤버를 반드시 구현해야 한다.");
        }
    }
